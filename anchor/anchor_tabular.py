...
class AnchorTabularExplainer(object):
    ...
    def get_sample_fn(self, data_row, classifier_fn, desired_label=None, mapping=None):
        def predict_fn(x):
        ...
        # construct mapping unless client wants to provide their own candidate anchors
        if mapping is None:
    ...
    def explain_instance(self, data_row, classifier_fn, threshold=0.95,
        ...
        # It's also possible to pass in mapping if user provides one
        sample_fn, mapping = self.get_sample_fn(
            data_row, classifier_fn, desired_label=desired_label, mapping=mapping)
        # return sample_fn, mapping
