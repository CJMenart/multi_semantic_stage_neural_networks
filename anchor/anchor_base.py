...
#         print 'lm', lm, 'qm', qm, kl_bernoulli(p, qm)
    ...
    def lucb(sample_fns, initial_stats, epsilon, delta, batch_size, top_n,
        ...
        # DEBUG DEBUG DEBUG
        print(f"lucb: top_n: {top_n}")
        print(f"lucb: n_samples.shape: {n_samples.shape}")
        print(f"lucb: means.shape: {means.shape}")
        print(f"lucb: n_features: {n_features}")
    ...
    def anchor_beam(sample_fn, delta=0.05, epsilon=0.1, batch_size=10,
        ...
        while current_size <= max_anchor_size:
            ...
            if len(tuples) == 0:
                # DEBUG
                print("Breaking search because all new anchors are worse.")
                break
